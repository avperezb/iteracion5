...
public class AllRentabilidadesMDB implements MessageListener, ExceptionListener {
	...
	public AllRentabilidadesMDB(TopicConnectionFactory factory, InitialContext ctx)
	...
	public ListaRentabilidades getRemoteRentabilidades() throws JsonGenerationException, JsonMappingException,
			JMSException, IOException, NonReplyException, InterruptedException, NoSuchAlgorithmException {
		answer.clear();
		...
		if (count == TIME_OUT) {
			if (this.answer.isEmpty()) {
				...
				System.out.println(answer.size() + "_=====");
			}
		}
		waiting = false;
		...
		ListaRentabilidades res = new ListaRentabilidades(answer);
		return res;
	}

	private void sendMessage(String payload, String status, Topic dest, String id)
			throws JMSException, JsonGenerationException, JsonMappingException, IOException {
		ObjectMapper mapper = new ObjectMapper();
		System.out.println(id);
		ExchangeMsg msg = new ExchangeMsg("rentabilidad.general.app2", APP, payload, status, id);
		...
		try {
			...
			if (!ex.getSender().equals(APP)) {
				if (ex.getStatus().equals(REQUEST)) {
					...
					ListaRentabilidades rentabilidades = dtm.getLocalRentabilidades(
							Long.parseLong(String.valueOf((int) ((Math.random() * 3) - 1))), Long.parseLong("1"),
							"01/01/17", "12/12/17");
					...
					ListaRentabilidades v = mapper.readValue(ex.getPayload(), ListaRentabilidades.class);
