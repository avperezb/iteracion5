	...
	public ArrayList<RentabilidadRestaurante> RFC14 (Long idPersona, RestauranteRentabilidad renta, Long idCosa) throws SQLException, Exception {
		...
		if (!buscarUsuarioPorID(idPersona).getRol().equals("Admin")){
			...
			if(idCosa == 1) {
				...
				String sqlCATEGORIA = "SELECT RESTAURANTES_PRODUCTOS.ID_RESTAURANTE, SUM(USUARIO_PEDIDO_PRODUCTOS.CANTIDAD),SUM(COSTO) AS SUMCOSTO, SUM(PRECIO) AS SUMPRECIO FROM PEDIDOS LEFT JOIN (((PRODUCTOS LEFT JOIN RESTAURANTES_PRODUCTOS ON PRODUCTOS.ID = RESTAURANTES_PRODUCTOS.ID_PRODUCTO) LEFT JOIN TIPO_COMIDA ON PRODUCTOS.TIPO = TIPO_COMIDA.ID) LEFT JOIN USUARIO_PEDIDO_PRODUCTOS ON USUARIO_PEDIDO_PRODUCTOS.ID_PRODUCTO_RESTAURANTE = RESTAURANTES_PRODUCTOS.ID_PRODUCTO) ON PEDIDOS.ID = USUARIO_PEDIDO_PRODUCTOS.ID_PEDIDO WHERE USUARIO_PEDIDO_PRODUCTOS.ID_PRODUCTO_RESTAURANTE = 2 AND USUARIO_PEDIDO_PRODUCTOS.ESTADO_PEDIDO ='ACEPTADO' AND PEDIDOS.FECHA BETWEEN to_date('"+renta.getFechaInicial()+"', 'DD/MM/YYYY') \r\n"
						+"AND to_date('"+renta.getFechaFinal()+"','DD/MM/YYYY')\r\n"
						+"GROUP BY RESTAURANTES_PRODUCTOS.ID_RESTAURANTE";
